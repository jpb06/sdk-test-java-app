...
(() => {
    ...
    var FramebusError = class extends Error {
        constructor(message, options) {
            super(message, options);
    ...
    var Framebus = class {
         ...
         */
        fetch(eventName, listener, payload, runOnce = false) {
            const internalListener = (payload2) => {
                if (runOnce) {
         ...
         * @param runOnce - Whether to remove the handler after the first response.
         */
        respond(eventName, callback, runOnce = false) {
         ...
         */
        parsePayload(eventName, payload) {
